...
def submit():
    ...
    sorted_prices = sorted(product_list, key=lambda x: int(x[1].replace(',', '').replace('원', '')))
    ...
    if alarm=="ntfy":
        ...
        return redirect(f"https://join.slack.com/t/pythonslack-lf58875/shared_invite/zt-2iig19ks4-X1JV6xPs58vMSR5NNEHQjg")
    else:
        asyncio.run(send_message.telegram_bot(id, message))
        return message
    

# 스레드를 이용하여 데이터베이스에 저장되어 있는 제품들 중 더 
# 저렴한 제품이 있는지 없는지 확인하는 함수
    ...
    while True:
        ...
        for idx, row in enumerate(ws.iter_rows(values_only=True)):
            ...
            if row!=('검색어', '제품명', '가격', '링크', 'id', 'app'):
                ...
                print(f"스레드 :: 이전에 {row[-2]}님이 검색한 제품 {row[0]}에 대한 최저값 검색을 시작합니다.")
                products_list = crawling_excel.danawa_crawling(row[0])
                products_list = products_list[:5]
                sorted_prices = sorted(products_list, key=lambda x: int(x[1].replace(',', '').replace('원', '')))

                data_price = int(row[2].replace(',', '').replace('원', ''))
                current_price = int(sorted_prices[0][1].replace(',', '').replace('원', ''))

                    # 지금 찾은 제품의 가격이 엑셀에 저장된 동일 제품의 가격보다 더 저렴한 경우 실행
                if current_price < data_price:

                        # 엑셀에 저장되어 있는 제품 데이터 업데이트
                    for i in range(2, 5):
                        ws.cell(row=idx+1, column=i).value = sorted_prices[0][i-2]
                ...
                else : print("스레드 :: 해당 제품의 더 싼 제품이 아직 없습니다.")
            wb.save(file_path)
            print("스레드 :: 데이터 업데이트 완료")
        # 주어진 시간 간격 동안 대기합니다.
